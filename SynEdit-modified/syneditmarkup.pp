...
type
  ...
  TSynEditMarkup = class(TComponent)
  private
  ...
  protected
    ...
    procedure SetSynEdit(AValue: TSynEditBase); virtual;
  ...
  public
    ...
    constructor Create(ASynEdit : TSynEditBase); overload; deprecated;
  ...
  published
    property SynEdit : TSynEditBase read FSynEdit write SetSynEdit;
...
procedure TSynEditMarkup.SetSynEdit(AValue: TSynEditBase);
begin
  if FSynEdit=AValue then Exit;
  if not (self is TSynEditMarkupManager) and assigned(FSynEdit) and (FSynEdit is TCustomSynEdit) then
  begin
    TCustomSynEdit(FSynEdit).MarkupManager.RemoveMarkUp(self);
  end;
  FSynEdit:=AValue;
  if not (self is TSynEditMarkupManager) and  assigned(FSynEdit) and (FSynEdit is TCustomSynEdit) then
  begin
    TCustomSynEdit(FSynEdit).MarkupManager.AddMarkUp(self);
  end;
end;
...
constructor TSynEditMarkup.Create(AOwner: TComponent);
begin
  inherited Create(AOwner);
  if AOwner is TSynEditBase then
    SetSynEdit(TSynEditBase(AOwner));
  FEnabled := true;
  FTempEnable := 0;
